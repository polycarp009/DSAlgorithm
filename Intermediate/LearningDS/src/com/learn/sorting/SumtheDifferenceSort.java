...
//for each subsequence, find the difference between the largest 
//and smallest numbers in that subsequence. Then add up all the differences to get the number.
public class SumtheDifferenceSort {
	public static void main(String[] args) {
	 ...
	 public static int solve(int[] A) {
		 ...
		 for (int i = 0; i < A.length; i++) {
			sum+=A[i]*((Math.pow(2, i))%m - (Math.pow(2, n-i-1))%m+m)%m;
            sum%=m;
		}
		return sum%m;
	    }
